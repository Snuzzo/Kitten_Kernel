...
static uint32_t wifi_off_gpio_table[] = {
	GPIO_CFG(HOLIDAY_GPIO_WIFI_IRQ, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_4MA), /* WLAN IRQ */
};
...
static int
holiday_wifi_status_register(void (*callback)(int card_present, void *dev_id),
				void *dev_id)
{
	if (wifi_status_cb)
		return -EAGAIN;
...
static int holiday_wifi_cd;	/* WiFi virtual 'card detect' status */

static unsigned int holiday_wifi_status(struct device *dev)
{
